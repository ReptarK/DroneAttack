...
namespace MyGame.Weapons
{
    public class M60 : Gun
    {
        ...
        const string NOM_ARME = "M60";
        const string NOM_TEXTURE_ARME = "m60";
        const string NOM_TEXTURE_SCOPED = "m60Scoped";

        const string NOM_GUN_SHOT = "M60GunShot"; //A CHANGER
        const string NOM_RELOAD = "M9Reload";

        const int DOMMAGE = 4;
        const int MUNITIONS = 100;
        const int TOTAL_MUNITIONS = 500;
        const float TEMPS_RELOAD = 5f;

        public const int PRIX = 8000;

        SoundEffect GunShot;
        SoundEffect ReloadSound;


        public M60(Game game)
            : base(game)
        {
            Prix = PRIX;
            Dommage = DOMMAGE;
        ...
        public override void Initialize()
        {
            ...
            ZoneAffichageScoped = new Rectangle(Ecran.CenterScreen.X - 298, Ecran.CenterScreen.Y - 14, 400, 250);
        ...
        public override void GererTirs(GameTime gameTime)
        {
            ...
            if (GestionInput.EstClicGauche() && CanShoot())
            {
                if (!LocalPlayer.EstScoped)
                {
                    ...
                    Game.Components.Add(new FadeTexture(Game, new Rectangle(Ecran.CenterScreen.X + 145, Ecran.CenterScreen.Y + 60, 45, 45), "blast", Color.White, 0.01f));
                }
